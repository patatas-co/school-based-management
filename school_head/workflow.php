...
$currentSY->execute([$syId]); $currentSY = $currentSY->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'save_milestone') {
        ...
        echo json_encode(['ok'=>true,'msg'=>'Milestone saved.']); exit;
    }
    ...
    if ($_POST['action'] === 'delete_milestone') {
        ...
        echo json_encode(['ok'=>true,'msg'=>'Milestone deleted.']); exit;
    }
    ...
    if ($_POST['action'] === 'save_cycle_schedule') {
        ...
        $hasAny = false;
        foreach ($phases as $p) { if ($p['start'] || $p['end']) { $hasAny = true; break; } }
        if (!$hasAny) { echo json_encode(['ok'=>false,'msg'=>'Please set at least one phase date.']); exit; }
        ...
        echo json_encode(['ok'=>true,'msg'=>'Cycle schedule saved successfully.']); exit;
    }
...
try {
    ...
    $ps->execute([$syId]);
    foreach ($ps->fetchAll() as $ph) $phaseSchedule[$ph['phase_no']] = $ph;
} catch (\Exception $e) { $phaseSchedule = []; }
...
foreach ($milestones as $m) $statusCount[$m['status']]++;
...
.wf-step:hover { box-shadow: var(--shadow-sm); }
.wf-step.active {
...
.wf-step-title  { font-size: 14px; font-weight: 700; color: var(--n900); margin-bottom: 4px; }
.wf-step-sub    { font-size: 12px; color: var(--n500); line-height: 1.5; }
.wf-arrow {
  display: flex; align-items: center; justify-content: center;
  color: var(--n300); font-size: 22px;
}
...
.milestone-title { font-size: 13.5px; font-weight: 700; color: var(--n900); margin-bottom: 3px; }
.milestone-desc  { font-size: 12.5px; color: var(--n500); line-height: 1.5; margin-bottom: 6px; }
.milestone-dates { font-size: 11.5px; color: var(--n400); display: flex; gap: 10px; flex-wrap: wrap; }
.milestone-actions { display: flex; gap: 5px; flex-shrink: 0; align-items: flex-start; }

/* ── Status colors ──────────────────────────────────────── */
.dot-upcoming    { background: var(--n300); }
.dot-in_progress { background: var(--blue); }
.dot-completed   { background: var(--brand-600); }
...
.ring-fill { fill: none; stroke: var(--brand-600); stroke-width: 8; stroke-linecap: round; transition: stroke-dashoffset .8s ease; }
.ring-text { position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); text-align: center; line-height: 1.2; }
.ring-pct  { font-family: var(--font-display); font-size: 18px; font-weight: 800; color: var(--n900); }
.ring-lbl  { font-size: 9.5px; font-weight: 600; color: var(--n400); }

/* ── Empty state ────────────────────────────────────────── */
.step-empty {
  text-align: center; padding: 28px 16px;
  color: var(--n400); font-size: 13px;
}
...
<div style="display:flex;align-items:center;gap:14px;padding:14px 20px;
  ...
  <span style="font-size:13px;font-weight:700;color:<?= $mat['color'] ?>;"><?= $cycle['overall_score'] ?>% — <?= $cycle['maturity_level'] ?></span>
  <?php endif; ?>
  <?php if($cycle['started_at']): ?>
  <span style="font-size:12px;color:var(--n500);margin-left:auto;">Started <?= date('M d, Y',strtotime($cycle['started_at'])) ?></span>
  <?php endif; ?>
  <?php if($cycle['submitted_at']): ?>
  <span style="font-size:12px;color:var(--n500);">Submitted <?= date('M d, Y',strtotime($cycle['submitted_at'])) ?></span>
  <?php endif; ?>
</div>
<?php endif; ?>
...
<div style="display:flex;align-items:center;gap:20px;margin-bottom:24px;flex-wrap:wrap;">
  ...
  <div class="ring-wrap">
    ...
    <svg viewBox="0 0 90 90" width="90" height="90">
      ...
      <circle class="ring-fill" cx="45" cy="45" r="37"
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;flex:1;">
    ...
    <div style="background:<?= $bg ?>;border-radius:var(--radius);padding:12px 16px;min-width:110px;text-align:center;">
      <div style="font-family:var(--font-display);font-size:24px;font-weight:800;color:<?= $color ?>;"><?= $statusCount[$key] ?></div>
      <div style="font-size:11.5px;font-weight:600;color:<?= $color ?>;margin-top:2px;"><?= $label ?></div>
    </div>
    <?php endforeach; ?>
    <div style="background:var(--n100);border-radius:var(--radius);padding:12px 16px;min-width:110px;text-align:center;">
      <div style="font-family:var(--font-display);font-size:24px;font-weight:800;color:var(--n700);"><?= $totalMilestones ?></div>
...
<div class="wf-steps" style="margin-bottom:28px;">
  ...
  <div class="wf-step <?= (!$cycle || $cycle['status']==='in_progress') ? 'active' : '' ?>">
    ...
    <div style="margin-top:10px;font-size:11.5px;font-weight:600;color:var(--brand-600);background:var(--brand-50);border-radius:6px;padding:4px 10px;display:inline-block;">
  ...
  <div class="wf-step <?= ($cycle && $cycle['status']==='submitted') ? 'active' : '' ?>">
    ...
    <div class="wf-step-sub">SDO validates the submitted assessment, reviews evidence, and provides official feedback.</div>
    ...
    <div style="margin-top:10px;font-size:11.5px;font-weight:600;color:var(--amber);background:var(--amber-bg);border-radius:6px;padding:4px 10px;display:inline-block;">
  ...
  <div class="wf-step <?= ($cycle && $cycle['status']==='validated') ? 'active' : '' ?>">
    ...
    <div style="margin-top:10px;font-size:11.5px;font-weight:600;color:var(--brand-700);background:var(--brand-100);border-radius:6px;padding:4px 10px;display:inline-block;">
  ...
  <div class="step-section-body">
    ...
    <div class="milestone-card" id="milestone<?= $m['milestone_id'] ?>">
      ...
      <div class="milestone-actions">
        <!-- Status quick-change -->
        <select class="fc" style="font-size:11.5px;padding:4px 8px;height:30px;width:auto;background:<?= $pillBg ?>;color:<?= $pillColor ?>;font-weight:600;border-color:<?= $pillColor ?>33;"
                onchange="updateStatus(<?= $m['milestone_id'] ?>,this.value)">
          <?php foreach(['upcoming','in_progress','completed','delayed'] as $sv): ?>
          <option value="<?= $sv ?>" <?= $m['status']===$sv?'selected':'' ?>><?= ucfirst(str_replace('_',' ',$sv)) ?></option>
          <?php endforeach; ?>
        </select>
        <?php if($_SESSION['role']==='school_head'): ?>
        <button class="btn btn-secondary btn-sm" onclick="editMilestone(<?= $m['milestone_id'] ?>)"><?= svgIcon('edit') ?></button>
        <button class="btn btn-danger btn-sm" onclick="delMilestone(<?= $m['milestone_id'] ?>)"><?= svgIcon('trash') ?></button>
        <?php endif; ?>
      </div>
    </div>
    <?php endforeach; ?>
...
<!-- DepEd Reference Box -->
<div style="margin-top:28px;padding:18px 20px;background:var(--n50);border:1px solid var(--n200);border-radius:var(--radius-lg);">
  <div style="font-size:12px;font-weight:700;color:var(--n400);text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px;">
