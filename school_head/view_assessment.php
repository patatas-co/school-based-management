...
.va-header-title { font-family: var(--font-display); font-size: 22px; font-weight: 800; margin-bottom: 6px; }
.va-header-sub   { font-size: 13px; color: rgba(255,255,255,.5); }
...
.ind-table th {
    background: var(--n50); padding: 8px 12px; text-align: left;
    font-size: 11px; font-weight: 700; color: var(--n500);
    border-bottom: 1px solid var(--n200); text-transform: uppercase; letter-spacing: .05em;
}
.ind-table td { padding: 10px 12px; border-bottom: 1px solid var(--n100); vertical-align: top; }
.ind-table tr:last-child td { border-bottom: none; }
.ind-table tr:hover td { background: var(--n50); }
.rating-pill {
...
<div class="va-header">
    <div style="position: relative; z-index: 1;">
        <div style="font-size: 11px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: rgba(74,222,128,.8); margin-bottom: 8px;">
            SBM Self-Assessment Report
        </div>
        <div class="va-header-title"><?= e($cycle['school_name']) ?></div>
        <div class="va-header-sub">School Year <?= e($cycle['sy_label']) ?> &nbsp;·&nbsp; <?= e($cycle['classification']) ?></div>

        <div style="display: flex; gap: 20px; margin-top: 16px; flex-wrap: wrap;">
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">Status</div>
                <span class="pill pill-<?= e($cycle['status']) ?>"><?= ucfirst(str_replace('_', ' ', $cycle['status'])) ?></span>
            </div>
            <?php if ($cycle['overall_score']): $mat = sbmMaturityLevel(floatval($cycle['overall_score'])); ?>
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">Overall Score</div>
                <div style="font-family: var(--font-display); font-size: 24px; font-weight: 800; color: #fff; line-height: 1;"><?= $cycle['overall_score'] ?>%</div>
            </div>
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">Maturity Level</div>
                <span style="display: inline-flex; padding: 3px 12px; border-radius: 999px; font-size: 12px; font-weight: 700; background: <?= $mat['bg'] ?>; color: <?= $mat['color'] ?>;"><?= e($cycle['maturity_level']) ?></span>
            </div>
            <?php endif; ?>
            <?php if ($cycle['school_head_name']): ?>
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">School Head</div>
                <div style="font-size: 13px; font-weight: 600; color: #fff;"><?= e($cycle['school_head_name']) ?></div>
            </div>
            <?php endif; ?>
            <?php if ($cycle['submitted_at']): ?>
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">Submitted</div>
                <div style="font-size: 13px; color: rgba(255,255,255,.7);"><?= date('M d, Y', strtotime($cycle['submitted_at'])) ?></div>
            </div>
            <?php endif; ?>
            <?php if ($cycle['validator_name']): ?>
            <div>
                <div style="font-size: 10.5px; color: rgba(255,255,255,.4); margin-bottom: 3px; text-transform: uppercase; letter-spacing: .06em;">Validated By</div>
    ...
    <div style="position: absolute; top: 20px; right: 24px; display: flex; gap: 8px; z-index: 1;">
        <a href="<?= baseUrl() ?>/export_pdf.php?cycle_id=<?= $cycleId ?>&type=annex_a"
           target="_blank"
           class="btn btn-sm"
           style="background: rgba(255,255,255,.12); color: #fff; border: 1px solid rgba(255,255,255,.2);">
            <?= svgIcon('download') ?> Annex A PDF
        </a>
        <a href="<?= baseUrl() ?>/export_pdf.php?cycle_id=<?= $cycleId ?>&type=dimension"
           target="_blank"
           class="btn btn-sm"
           style="background: rgba(255,255,255,.12); color: #fff; border: 1px solid rgba(255,255,255,.2);">
        ...
        <button class="btn btn-sm"
                style="background: rgba(220,38,38,.8); color: #fff; border: none;"
...
<div class="alert <?= $cycle['status'] === 'validated' ? 'alert-success' : 'alert-warning' ?>" style="margin-bottom: 18px;">
    ...
    <div class="card-body" style="padding: 16px 20px;">
        ...
        <div class="dim-score-row">
            ...
            <div style="flex: 1; min-width: 0;">
                <div style="font-size: 13px; font-weight: 600; color: var(--n800); margin-bottom: 4px;"><?= e($ds['dimension_name']) ?></div>
                <div style="height: 6px; background: var(--n100); border-radius: 999px; overflow: hidden;">
                    <div style="height: 100%; width: <?= min(100, $ds['percentage']) ?>%; background: <?= e($ds['color_hex']) ?>; border-radius: 999px;"></div>
                </div>
            </div>
            <div style="text-align: right; flex-shrink: 0; min-width: 110px;">
                <div style="font-size: 15px; font-weight: 800; color: <?= $mat['color'] ?>;"><?= number_format($ds['percentage'], 1) ?>%</div>
                <div style="font-size: 11px; color: var(--n400);"><?= $ds['raw_score'] ?>/<?= $ds['max_score'] ?> pts</div>
            </div>
            <span style="display: inline-flex; padding: 2px 9px; border-radius: 999px; font-size: 11px; font-weight: 600; background: <?= $mat['bg'] ?>; color: <?= $mat['color'] ?>; flex-shrink: 0;"><?= $mat['label'] ?></span>
        </div>
        <?php endforeach; ?>
    </div>
</div>
...
<div style="margin-bottom: 24px;">
    ...
    <div class="card" style="margin-bottom: 14px;">
        <div class="card-head" style="background: <?= e($first['color_hex']) ?>18; border-bottom: 2px solid <?= e($first['color_hex']) ?>40;">
            <div style="display: flex; align-items: center; gap: 8px;">
                <span style="width: 26px; height: 26px; border-radius: 6px; background: <?= e($first['color_hex']) ?>; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 800; flex-shrink: 0;"><?= $dimNo ?></span>
                <span class="card-title" style="color: <?= e($first['color_hex']) ?>;">Dimension <?= $dimNo ?>: <?= e($first['dimension_name']) ?></span>
            </div>
            <div style="display: flex; gap: 8px; font-size: 11px; flex-wrap: wrap;">
                <?php if ($dimRated1): ?><span style="background: #FEE2E2; color: #DC2626; padding: 2px 7px; border-radius: 999px; font-weight: 700;">NYM: <?= $dimRated1 ?></span><?php endif; ?>
                <?php if ($dimRated2): ?><span style="background: #FEF3C7; color: #D97706; padding: 2px 7px; border-radius: 999px; font-weight: 700;">EM: <?= $dimRated2 ?></span><?php endif; ?>
                <?php if ($dimRated3): ?><span style="background: #DBEAFE; color: #2563EB; padding: 2px 7px; border-radius: 999px; font-weight: 700;">DEV: <?= $dimRated3 ?></span><?php endif; ?>
                <?php if ($dimRated4): ?><span style="background: #DCFCE7; color: #16A34A; padding: 2px 7px; border-radius: 999px; font-weight: 700;">AM: <?= $dimRated4 ?></span><?php endif; ?>
            </div>
        </div>
        <div class="tbl-wrap">
            <table class="ind-table">
                ...
                <tr>
                    <td><span style="font-family: monospace; font-size: 11.5px; font-weight: 700; color: var(--n500);"><?= e($ind['indicator_code']) ?></span></td>
                    <td style="font-size: 12.5px; line-height: 1.55; color: var(--n800);"><?= e($ind['indicator_text']) ?></td>
                    <td style="font-size: 11.5px; color: var(--n400); font-style: italic; line-height: 1.4;"><?= e($ind['mov_guide'] ?? '—') ?></td>
                    <td>
