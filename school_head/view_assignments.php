...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] === 'get_assignments') {
            ...
            echo json_encode(['ok' => true, 'assignments' => $assignments]); exit;
        }
        if ($_POST['action'] === 'override_assignments') {
            $teacherId  = (int)$_POST['teacher_id'];
            $indicators = isset($_POST['indicators']) && is_array($_POST['indicators']) ? $_POST['indicators'] : [];
            $reason     = trim($_POST['reason'] ?? '');
            if (empty($reason)) throw new Exception("Reason for override is required.");
            ...
            if (!$verify->fetchColumn()) throw new Exception("Invalid teacher selected.");
            $db->beginTransaction();
            ...
            if (!empty($indicators)) {
                ...
                foreach ($indicators as $code) {
                    if (in_array($code, TEACHER_INDICATOR_CODES)) $insert->execute([$teacherId, $code, $_SESSION['user_id']]);
                }
            }
            ...
            $db->commit();
            echo json_encode(['ok' => true, 'msg' => 'Assignments overridden successfully!']); exit;
        }
    } catch (\Throwable $e) {
...
$pageTitle = 'Indicator Assignments'; $activePage = 'view_assignments.php';
...
?>
<style>
.teacher-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(380px,1fr));gap:16px;margin-bottom:24px;}
.teacher-card{background:white;border:1px solid var(--n-200);border-radius:12px;padding:16px;box-shadow:var(--shadow-xs);transition:all .2s ease;}
.teacher-card:hover{box-shadow:var(--shadow-sm);transform:translateY(-1px);}
.teacher-card-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;}
.teacher-info h3{margin:0 0 4px 0;font-size:16px;font-weight:700;color:var(--n-900);}
.teacher-info p{margin:0;font-size:13px;color:var(--n-500);}
.assignment-count{background:var(--purple-bg);color:var(--purple);padding:4px 8px;border-radius:6px;font-size:12px;font-weight:600;}
.current-assignments{background:var(--n-50);border:1px solid var(--n-200);border-radius:8px;padding:8px 12px;margin-bottom:12px;min-height:32px;}
.assignment-indicator{display:inline-block;background:var(--purple);color:white;padding:2px 6px;border-radius:4px;font-size:11px;font-weight:600;margin:2px;}
.indicator-group{margin-bottom:16px;}
.indicator-group-title{font-weight:600;color:var(--n-700);margin-bottom:8px;font-size:13px;}
.indicator-checkboxes{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;}
.indicator-checkbox{display:flex;align-items:center;gap:6px;font-size:12px;}
</style>
...
<div class="ph2">
  <div class="ph2-left">
    ...
    <div class="ph2-sub">View and manage teacher indicator assignments. You can override coordinator assignments when needed.</div>
  </div>
</div>
...
<div class="teacher-grid">
  ...
  <div class="teacher-card">
    ...
    <button class="btn btn-secondary btn-sm" onclick="showManageModal(<?= $teacher['user_id'] ?>, '<?= e($teacher['full_name']) ?>')">
...
<div class="card"><div class="card-body" style="text-align:center;padding:40px;"><h3 style="margin:0 0 8px 0;color:var(--n-600);">No Active Teachers Found</h3></div></div>
<?php endif; ?>

<div class="overlay" id="mManage">
  <div class="modal" style="max-width:600px;">
    <div class="modal-head"><span class="modal-title">Manage Assignments: <span id="modalTeacherName" style="color:var(--brand-600);"></span></span><button class="modal-close" onclick="closeModal('mManage')"><?= svgIcon('x') ?></button></div>
    <div class="modal-body">
      <input type="hidden" id="manage_teacher_id">
      <div class="fg"><label>Reason for Override <span style="color:var(--red);">*</span></label><textarea id="manage_reason" class="fc" rows="3" placeholder="Explain any changes…" required></textarea></div>
