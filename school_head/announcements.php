...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action']==='post') {
        ...
        echo json_encode(['ok'=>true,'msg'=>'Announcement posted.','ann'=>['id'=>$newId,'title'=>$title,'content'=>$content,'category'=>$cat,'target'=>$target,'author'=>$_SESSION['full_name']??'School Head']]); exit;
    }
...
?>
<style>
.ann-v2{background:var(--white);border:1px solid var(--n-200);border-radius:var(--radius-lg);overflow:hidden;box-shadow:var(--shadow-xs);margin-bottom:10px;transition:box-shadow 140ms,transform 140ms;}
...
<div class="status-tabs" style="margin-bottom:20px;">
  <a href="announcements.php" class="status-tab <?= !$filterCat ? 'active' : '' ?>">All <span class="status-tab-count"><?= array_sum($catCounts) ?></span></a>
  ...
  <a href="announcements.php?cat=<?= $c ?>" class="status-tab <?= $filterCat===$c ? 'active' : '' ?>">
    <span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:<?= $catColors[$c] ?>;margin-right:3px;"></span>
    <?= ucfirst($c) ?>
...
<?php if(!$anns): ?>
<div class="card"><div class="empty-state"><div class="empty-icon"><?= svgIcon('bell') ?></div><div class="empty-title">No announcements yet</div><button class="btn btn-primary" onclick="openModal('mPost')"><?= svgIcon('plus') ?> Post Announcement</button></div></div>
<?php else: ?>
<div id="annFeed">
  ...
  <div class="ann-v2-body">
    <div class="ann-v2-header">
      <div class="ann-v2-pills">
        ...
        <span class="pill" style="font-size:11px;background:var(--n-100);color:var(--n-600);"><?= ucfirst(str_replace('_',' ',$a['target_role'])) ?></span>
        <?php if($a['category']==='emergency'): ?><span class="pill" style="background:var(--red-bg);color:var(--red);border:1px solid #FECACA;font-size:11px;animation:pulse 1.2s infinite;">Urgent</span><?php endif; ?>
      </div>
    ...
    <div class="ann-v2-footer"><span>Posted by <strong style="color:var(--n-600);"><?= e($a['full_name']) ?></strong> · <?= timeAgo($a['created_at']) ?> · <?= date('M d, Y',strtotime($a['created_at'])) ?></span></div>
  </div>
</div>
<?php endforeach; ?>
</div>
