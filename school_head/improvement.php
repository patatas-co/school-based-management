...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($_POST['action'] === 'submit_ta_request') {
    ...
    if (!$concern || !$dimIds) {
      echo json_encode(['ok' => false, 'msg' => 'Please describe your concern and select at least one dimension.']);
      exit;
    }
    ...
    if ($existing->fetchColumn()) {
      echo json_encode(['ok' => false, 'msg' => 'You already have an active TA request. Please wait for the SDO to respond.']);
      exit;
    }
    ...
    echo json_encode(['ok' => true, 'msg' => 'TA request submitted successfully. The SDO will be notified.']);
    exit;
  }
...
if ($cycle) {
  ...
  $tq->execute([$schoolId, $cycle['cycle_id']]);
  $taRequests = $tq->fetchAll();
}
...
<div style="margin-top:28px;">
  ...
  <p style="font-size:13.5px;color:var(--n500);margin-bottom:18px;">
    Request coaching, mentoring, or monitoring support from the Schools Division Office. The SDO will review your SBM
    status and respond with recommended actions.
  </p>
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      <div class="card-body">
        ...
        <div class="grid2" style="gap:14px;margin-bottom:14px;">
          <div>
            <div style="font-size:11px;font-weight:700;color:var(--n400);text-transform:uppercase;margin-bottom:4px;">Your
              Concern</div>
            <div style="font-size:13.5px;color:var(--n800);line-height:1.6;"><?= nl2br(e($activeTaRequest['concern'])) ?>
            </div>
  ...
  <?php else: ?>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <?php if (!$cycle): ?>
          <div class="alert alert-warning"><?= svgIcon('alert-circle') ?> Complete your self-assessment first before
            requesting TA.</div>
        <?php else: ?>
          <div class="form-row" style="margin-bottom:14px;">
            <div class="fg">
              <label>Dimensions Needing Support * <span style="font-weight:400;color:var(--n400);">(select all that
                  apply)</span></label>
              <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:6px;" id="dimCheckboxes">
                <?php foreach ($dims as $d): ?>
                  <label class="dim-check-label">
                    <input type="checkbox" class="dim-checkbox" value="<?= $d['dimension_id'] ?>" onchange="updateDimIds()"
                      style="accent-color:<?= e($d['color_hex']) ?>">
          ...
          <div style="margin-top:6px;">
            ...
            <span style="font-size:12px;color:var(--n400);margin-left:12px;">The SDO will be notified and will respond to
              your request.</span>
          </div>
        <?php endif; ?>
      </div>
          ...
          <tbody>
            <?php foreach ($pastRequests as $req): ?>
              <tr>
                ...
                <td><span
                    class="pill pill-<?= $req['status'] === 'completed' ? 'validated' : 'returned' ?>"><?= ucfirst($req['status']) ?></span>
                </td>
