...
.search-icon { position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--n400); width:16px; height:16px; }
.search-input { width:100%; padding:10px 14px 10px 36px; border:1px solid var(--n200); border-radius:8px; font-size:13.5px; }
.teacher-table { width:100%; border-collapse:collapse; background:var(--white); border-radius:8px; overflow:hidden; box-shadow:var(--shadow-sm); }
.teacher-table th { background:var(--n50); font-size:12px; font-weight:700; color:var(--n500); text-align:left; padding:12px 16px; border-bottom:1px solid var(--n200); text-transform:uppercase; }
.teacher-table td { padding:14px 16px; border-bottom:1px solid var(--n100); font-size:13.5px; color:var(--n800); }
.teacher-table tr:hover { background:var(--n50); }
.teacher-name { font-weight:600; }
.teacher-email { font-size:12px; color:var(--n500); margin-top:3px; }

/* Modal specific overrides */
.indicator-group { margin-bottom:16px; border:1px solid var(--n200); border-radius:8px; overflow:hidden; }
.indicator-group-head { background:var(--n50); padding:10px 14px; font-size:13px; font-weight:700; color:var(--n700); border-bottom:1px solid var(--n200); display:flex; justify-content:space-between; align-items:center; }
.indicator-group-body { padding:12px 14px; display:flex; flex-direction:column; gap:8px; max-height: 250px; overflow-y:auto; }
.check-item { display:flex; align-items:flex-start; gap:10px; font-size:13px; cursor:pointer; }
.check-item input { margin-top:3px; }
.check-item div { flex:1; line-height:1.4; }
.check-badge { flex-shrink:0; font-weight:700; color:var(--blue); }
</style>
...
<div class="card" style="padding:20px;">
    <div class="toolbar">
        <form class="search-box" method="GET">
            ...
            <input type="text" name="q" class="search-input" placeholder="Search teacher by name or email..." value="<?= e($search) ?>" onchange="this.form.submit()">
        </form>
        <?php if($search !== ''): ?>
    ...
    <table class="teacher-table">
        ...
        <tbody>
            ...
            <tr><td colspan="3" style="text-align:center;padding:30px;color:var(--n500);">No active teachers found.</td></tr>
            <?php else: ?>
            ...
            <tr>
                ...
                <td>
                    ...
                    <span class="pill pill-success" style="font-size:12px;"><?= $t['assigned_count'] ?> of <?= count(TEACHER_INDICATOR_CODES) ?></span>
                    <?php else: ?>
                    <span class="pill pill-draft" style="font-size:12px;">Not assigned (Defaults to all)</span>
                    <?php endif; ?>
                </td>
                <td style="text-align:right;">
                    <button class="btn btn-secondary btn-sm" onclick="openAssignModal(<?= $t['user_id'] ?>, '<?= e(addslashes($t['full_name'])) ?>')">
...
<!-- Assign Modal -->
<div id="assignModal" class="modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
    <div class="modal-content" style="background:#fff;border-radius:12px;width:100%;max-width:700px;max-height:90vh;display:flex;flex-direction:column;box-shadow:var(--shadow-lg);">
        <div style="padding:20px;border-bottom:1px solid var(--n200);display:flex;justify-content:space-between;align-items:center;">
            <h3 style="font-size:18px;font-weight:700;">Assign Indicators: <span id="modalTeacherName" style="color:var(--brand-600);"></span></h3>
            <button onclick="closeModal()" style="background:none;border:none;cursor:pointer;color:var(--n500);"><?= svgIcon('x') ?></button>
        </div>
        ...
        <div style="padding:20px;overflow-y:auto;flex:1;background:var(--n50);">
            <form id="assignForm" onsubmit="saveAssignments(event)">
                ...
                <div style="margin-bottom:16px;font-size:13px;color:var(--n600);">
                    Select the specific indicators you want this teacher to evaluate. If you leave all unchecked, the teacher will default to evaluating ALL teacher-applicable indicators.
                </div>
                ...
                <div class="indicator-group" id="dimGroup<?= $dimId ?>">
                    <div class="indicator-group-head">
                        ...
                        <span style="font-size:12px;font-weight:600;color:var(--brand-600);cursor:pointer;" onclick="toggleDim(<?= $dimId ?>, true)">All</span>
                    </div>
                    <div class="indicator-group-body">
                        <?php foreach($dim['indicators'] as $ind): ?>
                        <label class="check-item">
                            <input type="checkbox" name="indicators[]" value="<?= $ind['code'] ?>" class="ind-chk dim-chk-<?= $dimId ?>">
        ...
        <div style="padding:16px 20px;border-top:1px solid var(--n200);display:flex;justify-content:flex-end;gap:10px;background:#fff;border-radius:0 0 12px 12px;">
