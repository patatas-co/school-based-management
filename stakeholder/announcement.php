...
$filterCat = $_GET['cat'] ?? '';
$sql = "SELECT a.*,u.full_name FROM announcements a JOIN users u ON a.posted_by=u.user_id WHERE a.target_role IN('all','external_stakeholder')";
$params = [];
...
// Count by category for the stakeholder part
$catCounts = $db->query("SELECT category, COUNT(*) cnt FROM announcements WHERE target_role IN('all','external_stakeholder') GROUP BY category")->fetchAll(PDO::FETCH_KEY_PAIR);

$pageTitle = 'Announcements'; $activePage = 'announcement.php';
...
<div class="ph2">
  <div class="ph2-left">
    ...
    <div class="ph2-sub">Recent notices and advisories.</div>
  </div>
</div>

<!-- Category filter tabs -->
<div class="status-tabs" style="margin-bottom:24px;">
  <a href="announcement.php" class="status-tab <?= !$filterCat ? 'active' : '' ?>">
    All <span class="status-tab-count"><?= array_sum($catCounts) ?></span>
  </a>
  <?php foreach(['general','policy','deadline','advisory','emergency'] as $c): ?>
  <a href="announcement.php?cat=<?= $c ?>" class="status-tab <?= $filterCat===$c ? 'active' : '' ?>">
...
<div class="ann-v2" id="ann<?= $a['ann_id'] ?>">
  ...
  <div class="ann-v2-body">
    ...
    <div class="ann-v2-footer">
      <span>Posted by <strong style="color:var(--n-600);"><?= e($a['full_name']) ?></strong> &middot; <?= timeAgo($a['created_at']) ?> &middot; <?= date('M d, Y',strtotime($a['created_at'])) ?></span>
    </div>
  </div>
</div>
<?php endforeach; ?>
