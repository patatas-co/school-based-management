    ...
    <div class="card-body">
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;">
        <?php foreach ($dimScores as $ds): ?>
          <div
            style="border:1px solid #E5E7EB;border-radius:9px;padding:14px;border-top:3px solid <?= e($ds['color_hex']) ?>;">
            <div style="font-size:12px;font-weight:700;color:var(--n500);margin-bottom:4px;">Dimension
              <?= $ds['dimension_no'] ?>
            </div>
            <div style="font-size:13.5px;font-weight:600;color:var(--n800);margin-bottom:10px;">
              <?= e($ds['dimension_name']) ?>
            </div>
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
              <span style="font-size:13px;color:var(--n600);">Score</span>
              <strong
                style="font-size:14px;color:<?= e($ds['color_hex']) ?>;"><?= number_format($ds['percentage'], 1) ?>%</strong>
            </div>
            <div class="prog">
              <div class="prog-fill"
                style="width:<?= min(100, $ds['percentage']) ?>%;background:<?= e($ds['color_hex']) ?>;"></div>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
