...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'clear_response') {
        ...
        if (!in_array($code, $assignedCodes)) {
            echo json_encode(['ok' => false, 'msg' => 'Not a teacher indicator.']);
            exit;
        }
...
")->execute([$cycleId, $uid, $cycleInfo['school_id'], $cycleInfo['sy_id'], $answered]);
    ...
    if ($_POST['action'] === 'save_response') {
        ...
        if (!in_array($code, $assignedCodes)) {
            echo json_encode(['ok' => false, 'msg' => 'You are not allowed to answer this indicator.']);
            exit;
        }
    ...
    <div class="dim-wrap" id="dim<?= $dimNo ?>" data-dim="<?= $dimNo ?>">
        ...
        <div class="dim-header" onclick="toggleDim(<?= $dimNo ?>)"
            ...
            <span style="font-size:11px;font-weight:600;border-radius:999px;
                     ...
                     transition:all .3s;" id="dimLeft<?= $dimNo ?>" <?php if ($allDone): ?>
                    style="color:#16A34A;background:#DCFCE7;border:1px solid #86EFAC;font-weight:700;" <?php else: ?>
                    style="color:var(--n500);background:var(--n100);" <?php endif; ?>>
...
<div style="text-align:center;padding:24px 0 32px;">
    ...
    <?php elseif (!$isLocked): ?>
        <button class="btn btn-primary" style="padding:12px 36px;font-size:15px;" id="submitBtn"
            ...
            <span id="submitCount" style="font-size:12px;opacity:.8;margin-left:4px;">
    ...
    async function submitMyAssessment() {
        ...
        if (r.ok) {
            ...
            if (btn) {
                btn.disabled = false;
                btn.textContent = '';
                btn.innerHTML = svgI('check') + ' Submit to School Head <span id="submitCount" style="font-size:12px;opacity:.8;margin-left:4px;">(' + prog.done + '/' + prog.total + ' rated)</span>';
            }
        }
    }
</script>
