...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action']==='save') {
        $db->prepare("UPDATE schools SET 
            school_name=?, school_id_deped=?, address=?,
            school_head_name=?, contact_no=?, email=?,
            total_enrollment=?, total_teachers=?
            WHERE school_id=?")
           ->execute([
        ...
        echo json_encode(['ok'=>true,'msg'=>'School profile updated.']); exit;
    }
    exit;
}

$school = $db->prepare("SELECT s.*, d.division_name FROM schools s LEFT JOIN divisions d ON s.division_id=d.division_id WHERE s.school_id=?");
$school->execute([SCHOOL_ID]); $school = $school->fetch();
...
$currentCycle  = $db->query("SELECT c.*, sy.label FROM sbm_cycles c JOIN school_years sy ON c.sy_id=sy.sy_id WHERE c.school_id=".SCHOOL_ID." ORDER BY c.created_at DESC LIMIT 1")->fetch();

$pageTitle = 'School Profile'; $activePage = 'school_profile.php';
...
<div class="ph2">
  <div class="ph2-left">
    ...
    <div class="ph2-sub">Manage DIHS institutional information and settings.</div>
  </div>
  <div class="ph2-right">
...
async function saveProfile(){
  const r=await apiPost('school_profile.php',{
    action:'save',
    school_name:$('s_name'), school_id_deped:$('s_deped'),
    address:$('s_addr'), school_head_name:$('s_head'),
    contact_no:$('s_contact'), email:$('s_email'),
    total_enrollment:$('s_enroll'), total_teachers:$('s_teachers')
  });
